...
if(!empty($_GET["fecha_inicial"]) and !empty($_GET["fecha_final"]))
{
   ...
   $consulta_reporte = $conexion->conexion->query("CALL SP_DESAPROBADOS_CIMEQH('$fecha_inicial','$fecha_final')");
   ...
   $pdf->Output("ReporteDesaprobadosCIMEQH-$fecha_inicial-$fecha_final.pdf", 'I');//nombreDescarga, Visor(I->visualizar - D->descargar)
 }
 else
 {
   echo("error");
