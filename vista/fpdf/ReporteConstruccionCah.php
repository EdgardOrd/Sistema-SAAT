...
if(!empty($_GET["fecha_inicial"]) and !empty($_GET["fecha_final"]))
{
   ...
   $consulta_reporte = $conexion->conexion->query("CALL SP_CONSTRUCCION_CAH('$fecha_inicial','$fecha_final')");
   ...
   $pdf->Output("ReporteOrdenadosPorConstruccionCAH-$fecha_inicial-$fecha_final.pdf", 'I');//nombreDescarga, Visor(I->visualizar - D->descargar)
 }
 else
 {
   echo("error");
